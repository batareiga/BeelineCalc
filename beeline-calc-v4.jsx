...
const TARIFFS = [
  { id:"abonement",  name:"Абонемент",      tag:"Один платёж в год",       color:"#FFE300", emoji:"📋",
    gb:1024, min:9999, sms:true,  price:395,  priceFull:750,  pT:395,  disc:47,  badge:"−47%",  hasMnp:false, isAnnual:true,
    b:{ h:"Один платёж — целый год спокойствия",     p:["Заплатил раз, забыл о платёжках навсегда","После года переходи на тариф дешевле","Только здесь доступен Красивый номер"] },
    s:{ h:"bee HIT · Оплата 4 740₽/год · −47%",     p:["1 024 ГБ · ∞ мин на все номера РФ · SMS","Экономия 3 060₽ по сравнению с помесячной оплатой","Доступен Красивый номер — только с этим тарифом"] } },
...
export default function App() {
  ...
  const [annual,setAnnual]=useState(false); const [tablet,setTablet]=useState(false);

  const prefs = {gb,min,budget,mnp,ai,yandex,cash,combo,kids,annual,tablet};

  const ranked = useMemo(()=>[...tariffs].map(t=>({...t,_s:scoreT(t,prefs)})).sort((a,b)=>b._s-a._s).slice(0,6), [tariffs,gb,min,budget,mnp,ai,yandex,cash,combo,kids,annual,tablet]);

  useEffect(()=>{ setAnimKey(k=>k+1); setSelectedId(null); },[gb,min,budget,mnp,ai,yandex,cash,combo,kids,annual,tablet]);
      ...
      <div style={{display:"flex",flexDirection:"column",height:"100vh"}}>
        ...
        <header style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:9}}>
            <div style={{width:32,height:32,background:"var(--y)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:900,color:"#000",letterSpacing:-1}}>B</div>
            <div>
              <div style={{fontSize:14,fontWeight:700,letterSpacing:-.2,lineHeight:1.1}}>Калькулятор выгод</div>
              <div style={{fontSize:10,color:"var(--t2)"}}>Билайн · {ranked.length} тарифов подобрано</div>
            </div>
          </div>
            ...
            <div style={{borderTop:"1px solid var(--b0)",paddingTop:16,marginTop:4}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:7}}>
                {[
                  ...
                  ["📋 Платить за год", annual, setAnnual, "#FFE300"],
              ...
              <div style={{display:"flex",borderBottom:"1px solid var(--b0)",flexShrink:0}}>
                {[["offers","✦ Предложения"],["script","💬 Скрипт"]].map(([id,label])=>(
                  <button key={id} onClick={()=>setTab(id)} className="btn" style={{
                    ...
                    border:"none",borderBottom:`2px solid ${tab===id?"var(--y)":"transparent"}`,
